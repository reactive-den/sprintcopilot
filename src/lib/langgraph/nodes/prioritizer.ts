...
export async function prioritizerNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
  ...
  const prompt = PRIORITIZER_PROMPT
    .replace('{tickets}', JSON.stringify(state.estimatedTickets, null, 2))
  ...
  try {
    ...
    return {
      currentStep: 'FAILED',
      errors: [...state.errors, `Prioritizer failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
    };
  }
}
