...
export async function estimatorNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
  ...
  const prompt = ESTIMATOR_PROMPT
    .replace('{tickets}', JSON.stringify(state.rawTickets, null, 2));
  ...
  try {
    ...
    return {
      currentStep: 'FAILED',
      errors: [...state.errors, `Estimator failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
    };
  }
}
